...
const InventoryCard = ({ item }: { item: ChemicalInventory }) => {
  const { consumeChemical, createPurchaseRequest } = useInventoryStore();
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 flex gap-2">
            ...
            {item.purchaseRequest && (
              <div className="flex-1 px-2 py-1.5 bg-yellow-500/20 text-yellow-400 rounded text-xs font-bold text-center flex items-center justify-center gap-1">
          ...
          {item.purchaseRequest && (
            <div className="mt-2 p-2 bg-yellow-500/10 rounded border border-yellow-500/30">
              <p className="text-xs text-yellow-400 flex items-center gap-1">
                <ShoppingCart className="w-3 h-3" />
                采购申请 #{item.purchaseRequest.id} - 申请数量: {item.purchaseRequest.quantity} {item.unit}
              </p>
              <p className="text-xs text-slate-500 mt-0.5">
                创建时间: {formatDateTime(item.purchaseRequest.createdAt)} | 预计到货: {item.purchaseRequest.estimatedArrivalDate}
              </p>
            </div>
          )}
              ...
              <div className="space-y-1 max-h-24 overflow-y-auto">
                {item.purchaseHistory.slice(0, 5).map((record, i) => (
                  <div key={i} className="text-xs text-slate-400 flex justify-between items-center">
                    <span>
                      <span className={record.status === 'completed' ? 'text-green-400' : 'text-yellow-400'}>
...
const InventoryPanel = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'low_stock' | 'normal'>('all');
      ...
      action={
        <div className="flex items-center gap-3 text-xs">
          <span className="text-slate-400">
            库存总值: <span className="text-yellow-400 font-mono font-bold">¥{formatNumber(totalValue, 0)}</span>
          </span>
    ...
    >
      <div className="grid grid-cols-4 gap-2 mb-3">
        ...
        <div className="p-2 bg-green-500/10 rounded-lg text-center">
          <p className="text-lg font-bold text-green-400 font-mono">{inventory.length - lowStockCount}</p>
        ...
        <div className="p-2 bg-yellow-500/10 rounded-lg text-center">
          <p className="text-lg font-bold text-yellow-400 font-mono">{pendingPurchaseCount}</p>
          <p className="text-xs text-slate-500">待采购</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-3">
        {[
          ...
          { key: 'normal', label: '正常', count: inventory.length - lowStockCount },
        ].map((item) => (
          <button
            key={item.key}
